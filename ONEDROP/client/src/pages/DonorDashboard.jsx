    ...
    try {
      ...
      if (targetEmail) {
        sendBloodRequestAlertEmail({
          ...
          hospital_name: matchesModalRequest?.hospitalName || 'Hospital',
                        ...
                        visibleRequests.filter(r =>
                          ...
                          <div key={req._id} className={`p-5 border rounded-2xl flex flex-col gap-4 shadow-sm hover:shadow-md transition-all ${
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                              ...
                              {/* Requester Info */}
                              <div className="flex-shrink-0 p-3 bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs space-y-1 min-w-[160px]">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Posted By</p>
                                <p className="font-extrabold text-slate-800 dark:text-white">{req.requester?.fullName || req.patientName}</p>
                                {(req.contactPhone || req.requester?.phone || req.phone) && (
                                  <p className="font-bold text-emerald-600 dark:text-emerald-400">📞 {req.contactPhone || req.requester?.phone || req.phone}</p>
                                )}
                              </div>
                            </div>

                            {/* Action Buttons */}
                            <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-200 dark:border-slate-700 items-center">
                              ...
                              {(req.contactPhone || req.requester?.phone || req.phone) && (
                                <>
                                  <a
                                    href={`tel:${req.contactPhone || req.requester?.phone || req.phone}`}
                                  ...
                                  >
                                    <Phone className="w-3.5 h-3.5" /> Call ({req.contactPhone || req.requester?.phone || req.phone})
                                  </a>
                                  <a
                                    href={`https://wa.me/${((req.contactPhone || req.requester?.phone || req.phone || '').replace(/\D/g, '').length === 10 ? '91' + (req.contactPhone || req.requester?.phone || req.phone || '').replace(/\D/g, '') : (req.contactPhone || req.requester?.phone || req.phone || '').replace(/\D/g, ''))}?text=${encodeURIComponent(`Hi, I saw your blood request for ${req.patientName} (${req.bloodGroup} needed) on ONEDROP. I want to help!`)}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-1.5 px-3 py-2 text-xs font-black text-white bg-[#25D366] hover:bg-[#1ebe59] rounded-xl transition-all shadow-sm"
                                  >
